...
install_command() {
    ...
    # Generate password if not provided and using mysql/mariadb
    if [[ -z "$MYSQL_PASSWORD" && "$database_type" != "sqlite" ]]; then
        MYSQL_PASSWORD=$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 20)
        colorized_echo yellow "Auto-generated database password: $MYSQL_PASSWORD"
    fi
    ...
    if [[ "$marzban_version" == "latest" || "$marzban_version" == "dev" || "$marzban_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$marzban_version"; then
            ...
            # Apply custom configurations
            {
                ...
                if [[ "$UVICORN_PORT" != "8000" ]]; then
                    echo "UVICORN_PORT=$UVICORN_PORT"
                fi
                if [[ -n "$MYSQL_PASSWORD" && "$database_type" != "sqlite" ]]; then
    ...
    sed -i '/--bind-address=127.0.0.1/s/127.0.0.1/0.0.0.0/' "$APP_DIR/docker-compose.yml"
